...
const MAIL_TOOL: Tool = {
  name: "mail",
  description: "Interact with Apple Mail app - create drafts, list emails, read emails, move emails, and list mailboxes.",
  inputSchema: {
    type: "object",
    properties: {
      operation: {
      ...
      isReply: {
        type: "boolean",
        description: "Set to true if creating a reply to an existing email (for createDraft operation)."
      },
      originalMessageId: { // Used by createDraft (if isReply)
        type: "string",
        description: "The ID of the original message if isReply is true (for createDraft operation)."
      },
      toAddress: {
        type: "string",
      ...
      searchTerm: {
        type: "string",
        description: "Text to search for in email subject and sender (optional, for list operation)."
      },
      limit: {
        type: "number",
        description: "Number of emails to retrieve (optional, for list operation, default 25)."
      },
      accountName: { // New for list
        type: "string",
        description: "The name of the account to list emails from (optional for list operation)."
      },
      mailboxName: { // New for list
        type: "string",
        description: "The name of the mailbox to list emails from (optional for list operation, defaults to 'Inbox')."
      },
      isRead: { // New for list
        type: "boolean",
        description: "Filter emails by read status (optional for list operation)."
      },
      isFlagged: { // New for list
        type: "boolean",
      ...
      targetAccountName: {
        type: "string",
        description: "The name of the account for the target mailbox (optional for move operation, recommended if mailbox name is not unique)."
      },
      // For listMailboxes
      // accountName: {
      //   type: "string",
      //   description: "The name of the account to list mailboxes for (optional for listMailboxes operation; if omitted, lists all mailboxes prefixed with account name)."
      // }
    },
    required: ["operation"]
  }
